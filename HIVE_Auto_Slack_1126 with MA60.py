...
while True:
    ...
    try:
        ...
        start_time = get_start_time("KRW-HIVE") + datetime.timedelta(hours=2)  # 11:00
        end_time = start_time + datetime.timedelta(days=1) - datetime.timedelta(hours=2)  # 9:00 <현재< #8:59:59

        if start_time < now < end_time - datetime.timedelta(seconds=10):
            target_price = get_target_price("KRW-HIVE", 0.1)
            #print("target_price:",target_price)
            current_price = get_current_price("KRW-HIVE")
            ma60 = get_ma60("KRW-HIVE")
            ...
            if target_price < current_price and current_price < ma60:
                ...
                if krw * 0.3 > 5000 and current_price < buy_price and  current_price < ma60:
                    ...
                    buy_result = upbit.buy_market_order("KRW-HIVE", krw * 0.4)
                    post_message(myToken, "#cointrade", "HIVE buy : " + str(buy_result['price']) + str(now)) if buy_result is not None else None
                    #print("buy_result:",buy_result)
                    buy_price = get_current_price("KRW-HIVE") if buy_result is not None else None  # 매수 금액을 buy_price에 입력
                    #print("buy_price_completed:", buy_price)
                    post_message(myToken, "#cointrade", "HIVE buy price: " + str(buy_price) + str(now))
                    # 매수 끝나고 나서 매도 대기
                    while buy_price is not None :
                        ...
                        now = datetime.datetime.now()
                        print(now)
                        if buy_price * 1.05 < current_price and earn * 0.995 > 3.001:
                            sell_result = upbit.sell_market_order("KRW-HIVE", earn * 0.995)
                            sell_price = get_current_price("KRW-HIVE") if sell_result is not None else None  # 매도 금액을 sell_price에 입력
                            post_message(myToken, "#cointrade", "HIVE 5Pro sell : " + str(float(sell_result['volume'])))
                            post_message(myToken, "#cointrade", "HIVE 5Pro sell : " + str(sell_price))
                            break
                        elif buy_price * 1.03 < current_price and earn * 0.995 > 3.001:
                            sell_result = upbit.sell_market_order("KRW-HIVE", earn * 0.9950)
                            sell_price = get_current_price("KRW-HIVE") if sell_result is not None else None  # 매도 금액을 sell_price에 입력
                            post_message(myToken, "#cointrade", "HIVE 3Pro sell : " + str(float(sell_result['volume'])))
                            post_message(myToken, "#cointrade", "HIVE 3Pro sell : " + str(sell_price))
                            break
                        elif start_time + datetime.timedelta(hours=1) < now and buy_price * 1.02 < current_price and earn * 0.995 > 3.001:
                            sell_result = upbit.sell_market_order("KRW-HIVE", earn * 0.9950)
                            sell_price = get_current_price("KRW-HIVE") if sell_result is not None else None  # 매도 금액을 sell_price에 입력
                            post_message(myToken, "#cointrade", "HIVE 2Pro sell : " + str(float(sell_result['volume'])))
                            post_message(myToken, "#cointrade", "HIVE 3Pro sell : " + str(sell_price))
                            break
                         ...
                         #   post_message(myToken, "#cointrade", "ETH 1.5Pro sell : " + str(float(sell_result['volume'])))
                         ...
                         #   sell_result = upbit.sell_market_order("KRW-ETH", earn * 0.9950)
                         #   sell_price = get_current_price("KRW-ETH") if sell_result is not None else None  # 매도 금액을 sell_price에 입력
                         #   post_message(myToken, "#cointrade", "ETH 1Pro sell : " + str(float(sell_result['volume'])))
                         #   post_message(myToken, "#cointrade", "ETH 1.5Pro sell : " + str(sell_price))
                         #   break
            ...
            # 하락세 급락 방지 손절
                ...
                if drop_price > current_price:
                    ...
                    if drop * 0.9 > 3.001:
                        ...
                        post_message(myToken, "#cointrade", "HIVE Drop sell : " + str(fail_result))
